...
describe("Load Testing Integration", () => {
  ...
  describe("High-Volume Operations", () => {
    test("should handle bulk space retrieval operations", async () => {
      ...
      console.log(
        `Bulk operations memory increase: ${(memoryIncrease / 1024 / 1024).toFixed(2)}MB`,
      );
      console.log(
        `Bulk operations execution time: ${executionTime.toFixed(2)}ms`,
      );
      console.log(
        `Average per operation: ${(executionTime / bulkOperations).toFixed(2)}ms`,
      );
    });
    ...
    test("should handle large batch search operations", async () => {
      ...
      console.log(
        `Batch search operations execution time: ${executionTime.toFixed(2)}ms`,
      );
      console.log(
        `Average per search: ${(executionTime / batchSize).toFixed(2)}ms`,
      );
    });
    ...
    test("should handle high-throughput page retrieval", async () => {
      ...
      const pageIds = Array.from(
        { length: throughputOperations },
        (_, index) => `load-test-page-${index + 1}`,
      );
      ...
      console.log(
        `High-throughput operations execution time: ${executionTime.toFixed(2)}ms`,
      );
  ...
  describe("Stress Testing", () => {
    test("should handle extreme concurrent load", async () => {
      ...
      const extremePromises = Array.from(
        { length: extremeLoad },
        (_, index) => {
      ...
      const memoryIncrease = memoryAfter - memoryBefore;
      console.log(
        `Extreme load memory increase: ${(memoryIncrease / 1024 / 1024).toFixed(2)}MB`,
      );
    ...
    test("should maintain stability under resource pressure", async () => {
      ...
      const pressurePromises = Array.from(
        { length: pressureOperations },
        (_, index) => {
          if (index % 3 === 0) {
            ...
            return handlers.createPage.handle({
              ...
              content: `<p>${"Large content block ".repeat(100)}</p>`.repeat(
                10,
              ),
            });
          }
      ...
      console.log(
        `Resource pressure execution time: ${executionTime.toFixed(2)}ms`,
      );
      console.log(
        `Stable operations under pressure: ${stableOperations}/${pressureOperations}`,
      );
    });
    ...
    test("should recover from breaking point scenarios", async () => {
      ...
      const breakingPromises = Array.from(
        { length: breakingPointOperations },
        (_, index) =>
          Promise.all([
      ...
      console.log(
        `Breaking point recovery time: ${executionTime.toFixed(2)}ms`,
      );
      console.log(
        `Recovered operations: ${recoveredOperations}/${breakingPointOperations}`,
      );
    });
  });
  ...
  describe("Memory Management", () => {
    test("should manage memory efficiently during sustained operations", async () => {
      ...
      const averageMemoryPerOperation =
        totalMemoryIncrease / sustainedOperations;
      expect(averageMemoryPerOperation).toBeLessThan(5 * 1024 * 1024); // Increased to 5MB per operation for test env

      console.log(
        `Sustained operations total memory increase: ${(totalMemoryIncrease / 1024 / 1024).toFixed(2)}MB`,
      );
      console.log(
        `Max memory usage: ${(maxMemoryUsage / 1024 / 1024).toFixed(2)}MB`,
      );
      console.log(
        `Average memory per operation: ${(averageMemoryPerOperation / 1024).toFixed(2)}KB`,
      );
    });
    ...
    test("should handle memory cleanup after large operations", async () => {
      ...
      const largeOperations = Array.from(
        { length: 10 },
        (
          _,
          index, // Reduced from 20
        ) =>
          handlers.createPage.handle({
            spaceId: `memory-space-${index}`,
            title: `Large Memory Page ${index}`,
            content: `<p>${"Memory intensive content ".repeat(100)}</p>`.repeat(
              5,
      ...
      const memoryAfterOperations = process.memoryUsage
        ? process.memoryUsage().heapUsed
        : 0;
      ...
      const memoryIncreaseAfterOperations =
        memoryAfterOperations - memoryBefore;
      ...
      console.log(
        `Memory increase after operations: ${(memoryIncreaseAfterOperations / 1024 / 1024).toFixed(2)}MB`,
      );
      console.log(
        `Memory increase after cleanup: ${(memoryIncreaseAfterCleanup / 1024 / 1024).toFixed(2)}MB`,
      );
      console.log(
        `Memory difference: ${(memoryRecovered / 1024 / 1024).toFixed(2)}MB`,
      );
    });
    ...
    test("should maintain stable memory usage during long-running operations", async () => {
      ...
      console.log(
        `Long-running memory growth: ${(memoryGrowth / 1024 / 1024).toFixed(2)}MB`,
      );
      console.log(
