...
describe("Performance Integration", () => {
  ...
  describe("Concurrent Operations Performance", () => {
    test("should handle multiple concurrent getSpaces requests efficiently", async () => {
      ...
      console.log(
        `Concurrent getSpaces (${concurrentRequests} requests) execution time: ${totalExecutionTime.toFixed(2)}ms`,
      );
      console.log(
        `Average per request: ${(totalExecutionTime / concurrentRequests).toFixed(2)}ms`,
      );
    });
    ...
    test("should handle mixed concurrent operations efficiently", async () => {
      ...
      console.log(
        `Mixed concurrent operations execution time: ${totalExecutionTime.toFixed(2)}ms`,
      );
    });
    ...
    test("should maintain performance under high concurrency load", async () => {
      ...
      const highConcurrencyPromises = Array.from(
        { length: highConcurrencyCount },
        (_, index) => {
      ...
      console.log(
        `High concurrency (${highConcurrencyCount} operations) execution time: ${totalExecutionTime.toFixed(2)}ms`,
      );
    });
  });
  ...
  describe("Resource Usage Performance", () => {
    test("should handle large parameter sets efficiently", async () => {
      ...
      const largeSearchResult = await handlers.searchPages.handle({
        query:
          'type = "page" AND (title ~ "test" OR title ~ "documentation" OR title ~ "guide" OR title ~ "tutorial")',
      ...
      console.log(
        `Large parameter search execution time: ${executionTime.toFixed(2)}ms`,
      );
    });
    ...
    test("should handle complex content operations efficiently", async () => {
      ...
      console.log(
        `Complex content creation execution time: ${executionTime.toFixed(2)}ms`,
      );
    });
    ...
    test("should maintain consistent performance across multiple operations", async () => {
      ...
      const averageTime =
        executionTimes.reduce((sum, time) => sum + time, 0) / operationCount;
  ...
  describe("Memory and Resource Management", () => {
    test("should handle rapid sequential operations without memory leaks", async () => {
      ...
      console.log(
        `Rapid sequential operations (${rapidOperationCount}) execution time: ${totalExecutionTime.toFixed(2)}ms`,
      );
    });
    ...
    test("should handle error scenarios without performance degradation", async () => {
      ...
      const averageErrorTime =
        executionTimes.reduce((sum, time) => sum + time, 0) / errorTestCount;
      ...
      console.log(
        `Error handling performance over ${errorTestCount} scenarios:`,
      );
      console.log(
