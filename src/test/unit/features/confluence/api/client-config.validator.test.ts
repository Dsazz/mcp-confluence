...
import { validateConfluenceConfig, isValidConfluenceConfig } from "../../../../../features/confluence/api/client-config.validator";
...
describe("validateConfluenceConfig", () => {
  ...
  describe("Invalid Configuration", () => {
    test("should throw error for empty host URL", () => {
      ...
      expect(() => validateConfluenceConfig(config)).toThrow("Configuration validation failed: Confluence host URL is required");
    });

    test("should throw error for empty API token", () => {
      const config = new ConfluenceConfig("https://test.com", "", "user@test.com");
      expect(() => validateConfluenceConfig(config)).toThrow("Configuration validation failed: Confluence API token is required");
    });

    test("should throw error for empty user email", () => {
      const config = new ConfluenceConfig("https://test.com", "token", "");
      expect(() => validateConfluenceConfig(config)).toThrow("Configuration validation failed: User email is required for API token authentication");
    });

    test("should throw error for multiple missing fields", () => {
      const config = new ConfluenceConfig("", "", "");
      expect(() => validateConfluenceConfig(config)).toThrow("Configuration validation failed: Confluence host URL is required, Confluence API token is required, User email is required for API token authentication");
    });

    test("should throw error for missing host URL and token", () => {
      const config = new ConfluenceConfig("", "", "user@test.com");
      expect(() => validateConfluenceConfig(config)).toThrow("Configuration validation failed: Confluence host URL is required, Confluence API token is required");
    });
  });
  ...
  describe("Edge Cases", () => {
    ...
    test("should accept URLs with paths", () => {
      const config = new ConfluenceConfig("https://test.com/confluence", "token", "user@test.com");
      expect(() => validateConfluenceConfig(config)).not.toThrow();
    });

    test("should accept URLs with ports", () => {
      const config = new ConfluenceConfig("https://test.com:8443", "token", "user@test.com");
      expect(() => validateConfluenceConfig(config)).not.toThrow();
    });

    test("should accept very long but valid tokens", () => {
      const longToken = "a".repeat(1000);
      const config = new ConfluenceConfig("https://test.com", longToken, "user@test.com");
      expect(() => validateConfluenceConfig(config)).not.toThrow();
    });

    test("should accept emails with plus addressing", () => {
      const config = new ConfluenceConfig("https://test.com", "token", "user+tag@test.com");
      expect(() => validateConfluenceConfig(config)).not.toThrow();
    });

    test("should accept emails with dots in local part", () => {
      const config = new ConfluenceConfig("https://test.com", "token", "first.last@test.com");
      expect(() => validateConfluenceConfig(config)).not.toThrow();
    });
